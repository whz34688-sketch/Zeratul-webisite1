(function () {
  ...
  function renderModule(module, section) {
    ...
    if (module.type === "chat-interface") {
      return renderChatModule(module);
    }
  ...
  function renderChatModule(module) {
    const quickQuestions = Array.isArray(module.quickQuestions) ? module.quickQuestions : [];
    const capabilityPills = Array.isArray(module.capabilityPills) ? module.capabilityPills : [];
    ...
    return `
      <section class="section-module">
        ...
        <div
          class="chat-layout"
          data-chat-root
          data-chat-api="${escapeAttribute(module.apiEndpoint || "/api/chat")}"
          data-chat-config="${escapeAttribute(module.configEndpoint || "/api/chat-config")}"
          data-chat-empty-title="${escapeAttribute(module.welcomeTitle || "Zeratul chat")}"
          data-chat-empty-text="${escapeAttribute(module.welcomeText || "")}"
          data-chat-scope="${escapeAttribute(module.scopeNote || "")}"
          data-chat-placeholder="${escapeAttribute(module.inputPlaceholder || "直接发给 Zeratul")}"
          data-chat-loading="${escapeAttribute(module.loadingText || "Zeratul 正在整理公开资料...")}"
          data-chat-send-label="${escapeAttribute(module.sendLabel || "发送")}"
          data-chat-clear-label="${escapeAttribute(module.clearLabel || "清空对话")}"
        >
          <aside class="content-card chat-guide-card">
            <p class="card-kicker">${module.eyebrow || "Knowledge Chat"}</p>
            <h3 class="card-title">${module.welcomeTitle || "Zeratul chat"}</h3>
            ${module.welcomeText ? `<p class="card-text">${module.welcomeText}</p>` : ""}
            ${module.scopeNote ? `<p class="chat-scope-note">${module.scopeNote}</p>` : ""}
            ${
              capabilityPills.length
                ? `<div class="pill-row chat-capability-row">${capabilityPills
                      ...
                      .map(
                        (question) => `
                          <button
                            ...
                            type="button"
                            data-chat-prompt="${escapeAttribute(question)}"
                          >
                            ${question}
                          </button>
                        `
            ...
            <div class="chat-runtime-note" data-chat-runtime-note>正在检查聊天服务配置...</div>
          </aside>

          <div class="content-card chat-console">
            <div class="chat-console-head">
              <div>
                <p class="card-kicker">Live Conversation</p>
                <h3 class="card-title">和 Zeratul 聊一聊</h3>
              </div>
              <button class="ghost-button chat-clear-button" type="button" data-chat-clear>
                ${module.clearLabel || "清空对话"}
              </button>
            </div>

            <div class="chat-message-list" data-chat-messages aria-live="polite"></div>
            <div class="chat-feedback-row" data-chat-feedback hidden></div>

            <div class="chat-composer">
              <label class="chat-input-label" for="zeratul-chat-input">发给 Zeratul</label>
              <textarea
                class="chat-input"
                id="zeratul-chat-input"
                rows="4"
                maxlength="1200"
                placeholder="${escapeAttribute(module.inputPlaceholder || "直接发给 Zeratul")}"
                data-chat-input
              ></textarea>
              <div class="chat-composer-foot">
                <div class="chat-turnstile-slot" data-chat-turnstile-slot hidden></div>
                <div class="action-row chat-composer-actions">
                  <button class="link-button chat-send-button" type="button" data-chat-send>
                    ${module.sendLabel || "发送"}
                  </button>
                </div>
              </div>
  ...
  function setupZeratulChat() {
    const chatRoots = Array.from(document.querySelectorAll("[data-chat-root]"));

    chatRoots.forEach((chatRoot) => {
      createZeratulChat(chatRoot);
    });
  }

  function createZeratulChat(chatRoot) {
    const runtimeNote = chatRoot.querySelector("[data-chat-runtime-note]");
    const messagesPanel = chatRoot.querySelector("[data-chat-messages]");
    const feedbackRow = chatRoot.querySelector("[data-chat-feedback]");
    ...
    const turnstileSlot = chatRoot.querySelector("[data-chat-turnstile-slot]");
    const promptButtons = Array.from(chatRoot.querySelectorAll("[data-chat-prompt]"));
    ...
    const state = {
      apiEndpoint: chatRoot.dataset.chatApi || "/api/chat",
      configEndpoint: chatRoot.dataset.chatConfig || "/api/chat-config",
      emptyTitle: chatRoot.dataset.chatEmptyTitle || "Zeratul chat",
      emptyText: chatRoot.dataset.chatEmptyText || "",
      scopeNote: chatRoot.dataset.chatScope || "",
      loadingText: chatRoot.dataset.chatLoading || "Zeratul 正在整理公开资料...",
      sendLabel: chatRoot.dataset.chatSendLabel || "发送",
      clearLabel: chatRoot.dataset.chatClearLabel || "清空对话",
      ready: false,
      loading: false,
      error: "",
      messages: [],
      runtimeNote: "正在检查聊天服务配置...",
      config: {
        ready: false,
        turnstile: {
          enabled: false,
    ...
    input.addEventListener("input", renderChatState);
    ...
    promptButtons.forEach((button) => {
      button.addEventListener("click", function () {
        submitQuestion(button.dataset.chatPrompt || button.textContent || "");
      });
    });
    ...
    async function loadChatConfig() {
      if (window.location.protocol === "file:") {
        state.runtimeNote = "Zeratul chat 需要通过本地服务运行。请使用 wrangler pages dev 打开网站。";
        state.ready = false;
        renderChatState();
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.error || "聊天配置读取失败。");
        }

        state.config = data;
        state.ready = Boolean(data.ready);
        state.runtimeNote = buildRuntimeNote(data);

        if (data.turnstile && data.turnstile.enabled && data.turnstile.siteKey) {
          turnstileSlot.hidden = false;
          await mountTurnstile(data.turnstile.siteKey);
        } else {
        ...
        state.runtimeNote =
          error instanceof Error && error.message
            ? error.message
            : "聊天配置读取失败，请确认当前站点已通过 Wrangler 本地服务运行。";
      }
    ...
    async function mountTurnstile(siteKey) {
      try {
        ...
        state.turnstile.widgetId = turnstile.render(turnstileSlot, {
          sitekey: siteKey,
          theme: "dark",
          ...
          "error-callback": function () {
            state.turnstile.token = "";
            state.error = "人机验证加载失败，请刷新页面后再试。";
            renderChatState();
          }
        });
      } catch {
        state.error = "Turnstile 脚本加载失败，请刷新页面后再试。";
      }
    }
    ...
    async function submitQuestion(question) {
      ...
      if (!state.ready) {
        state.error = state.runtimeNote || "聊天服务还没有准备好。";
        renderChatState();
        return;
      }

      if (state.config.turnstile && state.config.turnstile.enabled && !state.turnstile.token) {
        state.error = "请先完成人机验证，再发送问题。";
        renderChatState();
        return;
      }

      const historyPayload = buildHistoryPayload(state.messages);
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(formatChatError(data, response.status));
        }
        ...
        state.error =
          error instanceof Error && error.message ? error.message : "聊天请求失败，请稍后再试。";
      } finally {
        state.loading = false;
        resetTurnstileToken();
        renderChatState();
    ...
    function renderChatState() {
      ...
      const canSubmit = Boolean(!state.loading && state.ready && input.value.trim());

      runtimeNote.textContent = state.runtimeNote;
      runtimeNote.className = `chat-runtime-note${getRuntimeToneClass(state)}`;

      sendButton.textContent = state.loading ? "发送中..." : state.sendLabel;
      sendButton.disabled = !canSubmit;
      clearButton.textContent = state.clearLabel;
      clearButton.disabled = state.loading || (!hasMessages && !input.value.trim() && !state.error);
      input.disabled = state.loading || !state.ready;

      messagesPanel.innerHTML = hasMessages
        ? state.messages.map(renderChatMessage).join("") +
          (state.loading ? renderTypingMessage(state.loadingText) : "")
        : renderEmptyState(state);

      feedbackRow.hidden = !state.error;
      feedbackRow.textContent = state.error || "";
      feedbackRow.className = `chat-feedback-row${state.error ? " is-error" : ""}`;

      scrollMessagesToBottom(messagesPanel);
    }
    ...
    function resetTurnstileToken() {
      ...
      if (
        ...
        state.config.turnstile.enabled &&
  ...
  function ensureTurnstileScript() {
    ...
    turnstileScriptPromise = new Promise(function (resolve, reject) {
      ...
      window.__zeratulTurnstileOnload = function () {
        resolve(window.turnstile);
      };

      const script = document.createElement("script");
      script.src =
        "https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit&onload=__zeratulTurnstileOnload";
      script.async = true;
      script.defer = true;
  ...
  function renderEmptyState(state) {
    return `
      <article class="chat-empty-state">
        <span class="system-pill">Ready For Questions</span>
        <h4 class="chat-empty-title">${escapeHtml(state.emptyTitle)}</h4>
        <p class="card-text">${escapeHtml(state.emptyText)}</p>
        ${state.scopeNote ? `<p class="chat-empty-note">${escapeHtml(state.scopeNote)}</p>` : ""}
      </article>
    `;
  }
  ...
  function renderChatMessage(message) {
    ...
    return `
      <article class="chat-message${isUser ? " is-user" : " is-assistant"}">
        <div class="chat-message-meta">${isUser ? "访客" : "Zeratul"}</div>
        <div class="chat-message-body">
          ${isUser ? formatPlainText(message.content) : formatRichText(message.content)}
        </div>
      </article>
    `;
  }

  function renderTypingMessage(loadingText) {
    return `
      <article class="chat-message is-assistant is-loading">
        <div class="chat-message-meta">Zeratul</div>
        <div class="chat-message-body">
          ...
          <p class="card-text">${escapeHtml(loadingText)}</p>
        </div>
      </article>
    `;
  }
  ...
  function formatRichText(content) {
    ...
    return blocks
      .map(function (block, index) {
        ...
        return renderRichTextBlocks(block);
      })
      .join("");
  }
      ...
      .map(function (block) {
        ...
        if (lines.length && lines.every(function (line) { return /^\s*[-*]\s+/.test(line); })) {
          return `<ul class="chat-rich-list">${lines
            .map(function (line) {
              return `<li>${renderInlineMarkup(line.replace(/^\s*[-*]\s+/, ""))}</li>`;
            })
            .join("")}</ul>`;
        }

        if (lines.length && lines.every(function (line) { return /^\s*\d+\.\s+/.test(line); })) {
          return `<ol class="chat-rich-list ordered">${lines
            .map(function (line) {
              return `<li>${renderInlineMarkup(line.replace(/^\s*\d+\.\s+/, ""))}</li>`;
            })
            .join("")}</ol>`;
        }

        return `<p>${lines.map(renderInlineMarkup).join("<br />")}</p>`;
      })
      .join("");
  }
  ...
  function renderInlineMarkup(text) {
    ...
    output = output.replace(/`([^`]+)`/g, function (_, code) {
      return stashPlaceholder(placeholders, `<code>${escapeHtml(code)}</code>`);
    });

    output = output.replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, function (_, label, href) {
      return stashPlaceholder(
        placeholders,
        `<a href="${escapeAttribute(href)}" target="_blank" rel="noreferrer">${escapeHtml(label)}</a>`
      );
    });

    output = output.replace(/https?:\/\/[^\s<]+/g, function (href) {
      return stashPlaceholder(
        placeholders,
  ...
  function renderCodeBlock(block) {
    ...
    const hasLanguage = /^[a-z0-9.+#-]{1,20}$/i.test(firstLine);
    const language = hasLanguage ? firstLine : "";
    const code = hasLanguage ? lines.slice(1).join("\n") : trimmedBlock;

    return `
      <pre class="chat-code-block">
        ${language ? `<span class="chat-code-language">${escapeHtml(language)}</span>` : ""}
  ...
  function formatPlainText(content) {
    const safeLines = String(content || "").split("\n");
    return `<p>${safeLines.map(escapeHtml).join("<br />")}</p>`;
  }

  function stashPlaceholder(placeholders, html) {
    const token = `@@CHAT_PLACEHOLDER_${placeholders.length}@@`;
    placeholders.push(html);
    return token;
  }

  function restorePlaceholders(content, placeholders) {
    return placeholders.reduce(function (result, placeholder, index) {
      return result.replace(new RegExp(`@@CHAT_PLACEHOLDER_${index}@@`, "g"), placeholder);
    }, content);
  }
  ...
  function buildRuntimeNote(config) {
    if (!config.ready) {
      return "聊天接口已加载，但 AI 还没有配置完成。";
    }

    if (config.turnstile && config.turnstile.message) {
      return config.turnstile.message;
    }

    return "聊天服务已准备好，可以直接提问。";
  }

  function getRuntimeToneClass(state) {
    if (!state.ready) {
      return " is-warning";
    }

    if (state.config.turnstile && state.config.turnstile.enabled) {
      return " is-success";
    }
  ...
  function formatChatError(data, statusCode) {
    if (data && typeof data.error === "string" && data.error.trim()) {
      if (statusCode === 429 && typeof data.retryAfterMs === "number") {
        return `${data.error} 大约 ${Math.ceil(data.retryAfterMs / 1000)} 秒后再试。`;
      }
  ...
  function getOrCreateChatSessionId() {
    const storageKey = "zeratul-chat-session-id";
    ...
    try {
      ...
      const nextId =
        window.crypto && typeof window.crypto.randomUUID === "function"
          ...
          : `zeratul-${Date.now()}-${Math.random().toString(16).slice(2)}`;

      window.localStorage.setItem(storageKey, nextId);
      return nextId;
    } catch {
      return `zeratul-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    }
  }

  function scrollMessagesToBottom(messagesPanel) {
